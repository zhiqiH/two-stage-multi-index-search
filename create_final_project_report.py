...
REPORT_PATH = ROOT / "最终项目报告_轻量级多索引智能体搜索.docx"

DATASET_DIR = ROOT / "data_set"
ROUTER_AGENT_DIR = ROOT / "router_and_agent"
RESULTS_DIR = ROUTER_AGENT_DIR / "results" / "final"
VIS_DIR = ROUTER_AGENT_DIR / "visualization"
...
def load_manifest() -> dict:
    path = ROUTER_AGENT_DIR / "manifest" / "freeze_manifest.json"
    if path.exists():
...
def load_dataset_stats() -> dict[str, object]:
    questions_path = ROUTER_AGENT_DIR / "data" / "final" / "questions.jsonl"
    corpus_path = ROUTER_AGENT_DIR / "data" / "final" / "corpus.jsonl"
...
def build_report(rows: list[dict[str, str]], manifest: dict, dataset_stats: dict[str, object]) -> Document:
    ...
    add_para(
        doc,
        "本报告汇总整个项目的最终数据集、三类核心检索基线、路由器、两阶段智能体搜索方法、实验结果和可视化分析。报告中的实验结果均来自冻结目录 router_and_agent，最终智能体方法统一命名为 two_stage_agent。",
    )
    ...
    add_table(
        ...
        ["内容", "路径"],
        [
            ["冻结数据集", "router_and_agent/data/final"],
            ["冻结实验结果", "router_and_agent/results/final"],
            ["可视化图表", "router_and_agent/visualization"],
            ["最终 Agent 代码", "router_and_agent/code/src/agent.py"],
            ["Router 代码", "router_and_agent/code/src/router.py"],
            ["Evidence Judge 代码", "router_and_agent/code/src/evidence_judge.py"],
            ["冻结清单", "router_and_agent/manifest/freeze_manifest.json"],
        ],
    )
