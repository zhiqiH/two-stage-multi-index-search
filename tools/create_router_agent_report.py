...
REPORT_PATH = PROJECT_ROOT / "docs" / "reports" / "router_and_agent_design_and_results.docx"
MANIFEST_PATH = (
    PROJECT_ROOT
    / "results"
    / "analysis"
...
def build_report(rows: list[dict[str, str]], manifest: dict) -> Document:
    ...
    add_center(doc, "Frozen Router and Two-Stage Agent experiment")
    ...
    add_table(
        ...
        ["文件", "作用"],
        [
            ["src/router.py", "Rule Router：问题信号提取、file/graph/dense 分数、首轮工具选择。"],
            ["src/evidence_judge.py", "Evidence Judge：证据覆盖判断、停/继续决策、二轮工具选择、查询改写。"],
            ["src/agent.py", "Two-Stage Agent：路由、首轮检索、Judge、二轮检索、protected weighted RRF、预测输出。"],
            ["src/file_fts_search.py", "File FTS 检索器：FTS5、chunk 聚合、标题/短语/数字日期增强。"],
            ["src/graph_path_search.py", "Graph Path 检索器：实体文档图、路径 beam search、路径评分。"],
            ["src/dense_search.py", "Dense 检索器：MiniLM embedding、向量相似度检索。"],
            ["src/experiment_box.py", "反污染实验盒：公开运行视图、预测写出、gold-only 评价边界。"],
            ["results/final/", "冻结预测、逐题指标、聚合 summary 和 main_results.csv。"],
            ["experiment_records/", "汇报用 CSV 表、方法清单、运行命令记录。"],
            ["results/analysis/agent/freeze_manifest.original-layout.json", "原始冻结包的文件哈希、数据规模与方法列表。"],
        ],
    )
